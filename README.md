...
| 额外 NAS 授权目录 | 可选。已经在 fnOS/系统权限里授权给本应用运行用户访问的绝对路径 | `/vol1/游戏,/vol2/PS5` |

- PS5 连接端口固定 **9113**，无需填写。
- "电脑/手机 IP" **必须填**，否则打开网页时会显示 `loopback only` 错误。多台设备用英文逗号分隔，如 `192.168.1.100,192.168.1.101`。
- "额外 NAS 授权目录" 不会绕过 fnOS 权限；它只是把已经可读写的目录加入 WebUI 选择器。默认目录来自 fnOS `data-share` 授权。
...
如果你希望 WebUI 选择其它共享文件夹，先在 fnOS/系统权限中确保本应用运行用户可以访问该目录，再在应用配置里把路径填到“额外 NAS 授权目录”。
